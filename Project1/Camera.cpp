...
Camera::Camera(Vector3 position, Vector3 target, float FOV, float near, float far)
	: m_position(position)
	, m_target(target)
	, m_fov(FOV)
	, m_near_clip(near)
...
void Camera::SetPosition_Sperical(float radius, float azimuth, float polar){
	m_position = Vector3(
		radius*cos(polar)*cos(azimuth),
		radius*sin(azimuth),
		radius*cos(azimuth)*sin(polar));
		m_position += m_target;
}
